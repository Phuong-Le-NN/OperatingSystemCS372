...
void read_write_flash(int pickedSwapPoolFrame, int isWrite) {
    int devNo = currentP->p_supportStruct->sup_asid - 1;    /* ASID in range [1..8], devNo in range [0..7]*/
    int blockNo = swapPoolTable[pickedSwapPoolFrame].pgNo;

    /* Get the device register address for the U-proc’s flash device */
    device_t *flashDevRegAdd = (device_t *) devAddrBase(FLASHINT, devNo);
    ...
    /* Disable interrupts to ensure write COMMAND field and SYS5 atomically */
    ...
    SYSCALL(5, FLASHINT, devNo, FALSE);
 
    /* Re-enable interrupts */
    setSTATUS(getSTATUS() | IECBITON);
 
    if (flashDevRegAdd->d_status != 1){
...
void TLB_exception_handler() { /* 4.4.2 The Pager, Page Fault */
    ...
    setSTATUS(getSTATUS() & (~IECBITON)); /*step 11 and 12 atomically*/
